...
/// Draw Point as a Small Sphere so we can See it
void DrawPoint(const Point &p, float r = 1.0, float g = 1.0, float b = 1.0,
               float a = 1.0) {
  Draw(Round::sphere(p, .1), r, g, b, a);
}

/// Draw Null Pair as an Arrow at a Point
void DrawUnit(const Pair &p, float r, float g, float b) {
  Point pos = Round::location(p);
  DrawAt(-Round::dir(p).copy<Vec>().unit(), pos, r, g, b);
}

/// Draw Null Pair as a Line Segment at a Point
...
/// Draw TangentFrame -- see also draw (Frame)
...
void DrawLine(const TFrame &f) {
  DrawUnitLine(f.tu, 1, 0, .2);
  DrawUnitLine(f.tv, 0, 1, 0);
  DrawUnitLine(f.tw, 0, .4, 1);
}

/// Draw Curve from Point p
void DrawCurve(const Point &p, const Pair &log, int res, float r, float g,
               float b, float a = 1.0) {
  GL::lightsOff();
  glColor4f(r, g, b, a);
...
void DrawPoints(const Point &p, const Pair &logU, const Pair &logV, int res,
...
void DrawVolumeEdges(const TVolume &tf, float r = 0.0, float g = .7,
                     float b = 1.0) {
