  ...
  if (fabs (ta) > FPERROR)
    {
      ...
      (real)
        ? gfx::Glyph::SolidSphere (t, 5 + floor (t * 30), 5 + floor (t * 30))
        : gfx::Glyph::Sphere (t);
    }
  else
    {
...
void ColorTriangles (const T &a, const T &b, const T &c)
{
  ...
  auto normal =
    -NEVec<3> (
       (nga::Euc::hom (a) ^ nga::Euc::hom (b) ^ nga::Euc::hom (c)).dual ())
       .unit ();

  gfx::GL::normal (normal.begin ());
  glBegin (GL_TRIANGLES);
  glColor3f (1, 0, 0);
  gfx::GL::vertex (a.begin ());
  glColor3f (0, 1, 0);
  gfx::GL::vertex (b.begin ());
  glColor3f (0, 0, 1);
  gfx::GL::vertex (c.begin ());
  glEnd ();
}
...
void DrawAt (const A &s, const B &p, float r = 1, float g = 1, float b = 1,
             float a = 1)
{
  glPushMatrix ();
...
//Alt
template <class A, class B>
void DrawAtB (const A &s, const B &p, float r = 1, float g = 1, float b = 1,
              float a = 1)
{
  glPushMatrix ();
  glTranslatef (p[0], p[1], p[2]);
