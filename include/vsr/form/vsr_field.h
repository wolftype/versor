...
#define ITN for (int i = 0; i < this->mNum; ++i) {
#define END }
...
///  A Basic 3D Field (slowly porting this over from the now defunct vsr_lattice
///  class)
...
class Field : public CubicLattice<typename T::space::point> {
 ...
 public:
  ...
  Field(int w = 1, int h = 1, int d = 1, double spacingX = 1.0,
        double spacingY = 1.0, double spacingZ = 1.0)
      : CubicLattice<GridType>(w, h, d, spacingX, spacingY, spacingZ),
        mData(NULL)
  ...
  Field& resize(int w, int h, int d, double spacingX = 1.0,
                double spacingY = 1.0, double spacingZ = 1.0) {
  ...
  T at(int w = 0, int h = 0, int d = 0) const {
    return mData[this->idx(w, h, d)];
  }

  // INITIALIZE
  void basicInit() {
    ITER mData[tidx] =
        this->mPoint[tidx].template copy<T>();  // T( this->mPoint[ tidx ] );
    ITEND
  }
  // SPECIALIZE HERE
  void init() { basicInit(); }
  ...
  //        Vec rand(){
  //            return Vec( px(Rand::Int(mWidth)) , py(Rand::Int(mHeight)),
  //            pz(Rand::Int(mDepth))  );
  //        }
  //
  ...
  // Create Vector Derivative field of another Field f. . . e1 * dx + e2 * dy +
  // e3 * dz . . . in other dimensions use reciprocal frame
  //  template<class B>
  ...
  double tensNbrs(int idx) const {
    double tdx;
    tdx += mData[this->nbr(idx).xr][0] - mData[this->nbr(idx).xl][0];  // xr -
                                                                       // xl
    tdx += mData[this->nbr(idx).yt][1] - mData[this->nbr(idx).yb][1];  // yt -
                                                                       // yb
    tdx += mData[this->nbr(idx).zb][2] - mData[this->nbr(idx).zf][2];  // zb -
                                                                       // zf
    return tdx;
  }
  ...
  double tensNbrsWt(int idx) const {
    ...
    tdx += (mData[this->nbr(idx).xr][0] - mData[this->nbr(idx).xl][0]) *
           ww;  // xr - xl
    tdx += (mData[this->nbr(idx).yt][1] - mData[this->nbr(idx).yb][1]) *
           wh;  // yt - yb
    tdx += (mData[this->nbr(idx).zb][2] - mData[this->nbr(idx).zf][2]) *
           wd;  // zb - zf
    return tdx;
  }
  ...
  /*! Backwards Advection Using a Previous Field State prev and Based on a
   * Velocity Frame f */
  template <class B>
  void advect(const Field& prev, const Field<B>& f, double dt, bool ref) {
    ...
    BOUNDITER
    int tidx = this->idx(i, j, k);
    auto p = this->mPoint[tidx] +
             f.euler3d(this->mPoint[tidx]) *
                 -dt0;  // .trs( f.euler3d( mPoint[tidx] ) * -dt0 );//f[tidx] *
                        // -dt0 ); //Lattice Point
  ...
  /* Field& operator -= ( const Field< MV<XS...> >& f ) { ITN mData[i] -= f[i];
   * END  return *this; } */
...
//
//             //negation for now treat as vectors
//             if (type & LEFT) mData[ix][0] = ref ? -mData[n.xr][0] :
//             mData[n.xr][0]; if (type & RIGHT) mData[ix][0] = ref ?
//             -mData[n.xl][0] : mData[n.xl][0]; if (type & TOP) mData[ix][1] =
//             ref ? -mData[n.yb][1] : mData[n.yb][1]; if (type &
//             BOBits::TypeOM) mData[ix][1] = ref ? -mData[n.yt][1] :
//             mData[n.yt][1]; if (type & BACK) mData[ix][2] = ref ?
//             -mData[n.zf][2] : mData[n.zf][2]; if (type & FRONT) mData[ix][2]
//             = ref ? -mData[n.zb][2] : mData[n.zb][2];
//         }
//     }
//
//
...
//       ITN  GL::push(); glColor4f(r,g,b,mData[i][0] * a);
//       GL::translate(mPoint[i].w()); GL::Glyph::Cube( mSpacing ); GL::pop();
//       END
//    }
...
//    double tdx;
//    tdx += mData[ nbr(idx).xr ].pos()[0] - mData [ nbr(idx).xl ].pos()[0];
//    //xr - xl tdx += mData[ nbr(idx).yt ].pos()[1] - mData [ nbr(idx).yb
//    ].pos()[1]; //yt - yb tdx += mData[ nbr(idx).zb ].pos()[2] - mData [
//    nbr(idx).zf ].pos()[2]; //zb - zf return tdx;
//  }
