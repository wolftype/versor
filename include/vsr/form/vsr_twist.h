...
class TwistField {
 ...
 public:
  ...
  /// Place cube of Frames on corners of a field of points, and register
  /// mDistance of points
  ...
  vector<Pnt> apply(vector<Pnt>& pnt) {
    ...
    for (int i = 0; i < mDistance.size(); ++i) {
      ...
      auto tdll =
          mDll.vol(dis[0], dis[1], dis[2]);  // get trilinear interp of field
...
class Twist {
 ...
 public:
  ...
  void set(double distance, double period) {
    Mot m = Gen::trs(Vec(drv()).unit() * distance) *
            Gen::rot(biv().unit() * period);
    m /= m.rnorm();
    mDll = Gen::log(m);
  }
  ...
  double pitch() const {
    ...
    // return Op::sca( Vec(mDrv).unit() <= Op::dle(mBiv).unit() ); //Op::pj(
    // Vec(mDrv), Op::dle(mBiv) ).rnorm();
  }
...
inline ostream& operator<<(ostream& os, const Twist& t) {
  os << t.dll() << " pitch: " << t.pitch() << " period: " << t.period()
     << " moment: " << t.moment();
  return os;
}
...
class CoupledTwist {
 ...
 public:
  ...
  /// Averaged Generators
  Dll dll() {
    return (mTwist[0].dll() + mTwist[1].dll() + mTwist[2].dll()) / 3.0;
  }
...
class NTwist {
 ...
 public:
  ...
  Dll at(double t) { return Interp::quadric<Dll>(mDll, mNum, t); }
};
