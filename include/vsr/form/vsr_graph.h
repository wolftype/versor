...
struct HEGraph
{
  ...
  struct HalfEdge
  {
    ...
    HalfEdge ()
        : node (NULL), face (NULL), opp (NULL), next (NULL), bVisited (false)
    {
    }
    ...
    {
      return *(next->next);
    }  ///< Reference to Previous Edge (clockwise)

    T &a () { return node->data (); }  ///< Reference to T data at incident node
    T &b ()
    {
      return prev ().node->data ();
    }  ///< Reference to T data at other end

    bool isBorder ()
    {
      return opp == NULL;
    }  ///< Whether edge is on the edge of the manifold
    ...
    {
      if ((node == eb.prev ().node) && (prev ().node == eb.node))
        return true;
      return false;
    }
    ...
    bool triangle ()
    {
      ...
      if (&nextNull (true) == &(eb.nextNull (false)))
        return true;
      else
        return false;
    }
  ...
  struct Face
  {
    ...
    vector<Face *> edgeNeighbors ()
    {
      ...
      if (ea ().opp != NULL)
        vp.push_back (ea ().opp->face);
      if (eb ().opp != NULL)
        vp.push_back (eb ().opp->face);
      if (ec ().opp != NULL)
        vp.push_back (ec ().opp->face);
  ...
  void seedNodes ()
  {
    ...
    ea->next = eb;
    ea->face = f;  // assign next edge and face
    eb->next = ec;
    eb->face = f;
    ec->next = ea;
    ec->face = f;
  ...
  HEGraph &add (T &v)
  {
    ...
    if (num < 3)
      addNode (v);
    if (num == 2)
      seedNodes ();  //if num was 2 coming in, it is now 3 so seed
    if (num >= 3)
      addAt (v, lastEdge ());
    return *this;
  }
  ...
  HEGraph &addAt (T &v, int x) { return addAt (v, edge (x)); }
...
template <class T, class E>
inline vector<typename HEGraph<T, E>::HalfEdge *>
HEGraph<T, E>::Node::valence () const
{
  vector<HEGraph::HalfEdge *> tmp;
...
template <class T, class E>
inline vector<typename HEGraph<T, E>::HalfEdge *>
HEGraph<T, E>::Node::edgeLoop () const
{
  vector<HEGraph::HalfEdge *> tmp;
  auto v = valence ();
  for (auto &i : v)
...
inline vector<typename HEGraph<T, E>::Node *> HEGraph<T, E>::Node::neighbors ()
{
  ...
  if (!closed ())
    tmp.push_back (res.back ()->next->node);
  return tmp;
}
...
template <class T, class E>
template <class S>
inline HEGraph<T, E> &HEGraph<T, E>::UV (int w, int h, S *p, bool bCloseU,
                                         bool bCloseV)
{
  HEGraph &graph = *this;
  graph.clear ();

  //Add first column of points to graph
  for (int j = 0; j < h; ++j)
    {
      if (j == 0)
        {
        ...
        {
          graph.addAt (p[j], graph.edge (-3));
          graph.addAt (p[h+j], graph.edge (-1));
        }
    }
  //Add rest
  for (int i = 2; i < w; ++i)
    { int pidx = h*i;
      ...
      graph.addAt (p[pidx+1], graph.edge (-3));
      for (int j = 2; j < h; ++j)
        {
          int tidx = h*i+j;
