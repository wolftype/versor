...
struct CyclideQuad{
  ...
  Pair mLogU, mLogV;      ///<-- curvature tensors across surface
  Pair mLogUz, mLogVz;    ///<-- two more (normals)
  ...
  void set( const TangentFrame& ta, const TangentFrame& tb, const TangentFrame& tc, const TangentFrame& td){
    tframe[0] = ta.unit(); tframe[1] = tb.unit(); tframe[2]=tc.unit(), tframe[3]=td.unit();
    calcSurfaces();//ta, false, false, false, false, 0);
    log();
  }
  ...
  void set( const Contact& a, const Contact& b, const Contact& c, const Contact& d, float spin = 0 ){
    ...
    calcFrame (TangentFrame(Frame(a.point, rot2) ), false, false, false, false, 0);
  ...
  void calcFrame(){
    ...
    //FIRST ONE
    tframe[0].bitan[0] = tframe[0].tx().undual(); //undual???
    ...
    for (int i=1;i<4;++i){
     int next = ( i < 3 ) ? i+1 : 0;
     for (int j=0;j<3;++j){
       tframe[i].bitan[j] = Tangent::at( tframe[i-1].sphere[j], tframe[i].pos() );
...
//    for (int i=0;i<4;++i) tframe[i].calcCurves();
  ...
  void log(){
     ...
     // mLogU = Gen::log( - tframe[2].sphere[0].dual() / tframe[0].sphere[0].dual() ), altU() );
      ...
      mLogUy = Gen::log( Gen::ratio(tframe[0].sphere[1].dual(), tframe[2].sphere[1].dual(), true ), altYu() );
      // log in v direction  (across constant x)
      mLogVx = Gen::log( Gen::ratio(tframe[3].sphere[0].dual(), tframe[1].sphere[0].dual(), true ), altXv() );

    //  auto tmp = (tframe[0].sphere[0].dual() / tframe[2].sphere[0].dual()).runit();
    //  cout << tmp[0] << endl;
  ...
  /// Calculate logs based on ratios of constant coordinate surface spheres
  /// used for sense of interpolation testing do not use (use log() instead )
  // void log2(
  // bool bNegA=true, bool bNegB=true,
  // bool bNegC=false, bool bNegD=false,
  // bool bNegE=false,bool bNegF=false,
  // bool bNegG=false, bool bNegH=false){
  //
  //     // log in u direction -- ratio of surfaces of constant u (sphere[0])
  //   //  mLogU = Gen::log( Gen::ratio(tframe[0].sphere[0].dual(), tframe[2].sphere[0].dual(), true ), false );
  //     // log in v direction -- ratio of surfaces of constant v (sphere[1])
  //   //  mLogV = Gen::log( Gen::ratio(tframe[3].sphere[1].dual(), tframe[1].sphere[1].dual(), true ), false );
  //
  //     auto tmpU = (tframe[2].sphere[0].dual() / tframe[0].sphere[0].dual() * (bNegA?-1:1) ).runit();// * (bNegA?-1:1);
  //     auto tmpV = (tframe[1].sphere[1].dual() / tframe[3].sphere[1].dual() * (bNegB?-1:1)).runit();// * (bNegB?-1:1);
  //   //  auto tmpU = Gen::ratio(tframe[0].sphere[0].dual(), tframe[2].sphere[0].dual(), bNegA );
  //   //  auto tmpV = Gen::ratio(tframe[3].sphere[1].dual(), tframe[1].sphere[1].dual(), bNegB );
  //
  //   //  if (altU()) tmpU = (tframe[2].sphere[0].dual() / tframe[0].sphere[0].dual()  ).runit()* (bNegA?-1:1);
  //   //  if (altV()) tmpV = (tframe[1].sphere[1].dual() / tframe[3].sphere[1].dual()  ).runit()* (bNegB?-1:1);
  //
  //     cout << "debug u: " << altU() << " v: " << altV() << endl;
  //   //  cout << (bU>0) << " " << (bV>0) << endl;
  //     mLogU = Gen::log( tmpU, bNegC, bNegE  ) * .5 * (bNegG?-1:1);
  //     mLogV = Gen::log( tmpV, bNegD, bNegF )  * .5 * (bNegH?-1:1);
  //
  //
  // }
  ...
  void logZ(){
      mLogUz = Gen::log( Gen::ratio( tframe[0].sphere[2].dual(), tframe[2].sphere[2].dual(),true ), altWU() );
      mLogVz = Gen::log( Gen::ratio( tframe[3].sphere[2].dual(), tframe[1].sphere[2].dual(),true ), altWV() );
  }
  ...
  Pair ortho(VSR_PRECISION u, VSR_PRECISION v){
      return tframe[0].sphere[2].dual().spin( xfuz(u) ) ^ tframe[3].sphere[2].dual().spin( xfvz(v) );
  }
  ...
  Con xfxz( VSR_PRECISION u, VSR_PRECISION v ) const { return xfvz(v) * xfu(u); }
  Con xfyz( VSR_PRECISION u, VSR_PRECISION v ) const { return xfuz(u) * xfv(v); }

  /// Circles in U direction
  Circle cirU( VSR_PRECISION u) { return tframe[0].calcCurve(1).spin( xfu(u) ); }
  /// Circles in V direction
  Circle cirV( VSR_PRECISION v) { return tframe[3].calcCurve(0).spin( xfv(v) ); }
...
//  /// Apply conformal rotor to a type T at u,v
  ...
  void calcFrame(const TangentFrame& f, bool bFlipX, bool bFlipY, bool bCorner, bool bNeg, int begin){
    ...
    for (int i=1;i<4;++i){
     ...
     for (int j=0;j<3;++j){
       tframe[cur].bitan[j] = Tangent::at( tframe[prev].sphere[j], tframe[cur].pos() );
  ...
  /// bNeg: whether to flip the direction
   CyclideQuad attach(const DualSphere& sphere, int edge, bool bCorner=false, bool bNeg=false){
       ...
       switch( edge ){
         case 0:
         {
           ...
           auto cd = Contact( cyc.tframe[0].pos(), cyc.tframe[0].sphere[2].dual() );
           if (bNeg) cd.tnv *= -1;
           auto ca = Contact( cd, sphere );
           auto cc = Contact( cyc.tframe[1].pos(), cyc.tframe[0].sphere[2].dual() );
           if (bNeg) cc.tnv *= -1;
         ...
         case 1:
         {
           ...
           auto ca = Contact( cyc.tframe[1].pos(), cyc.tframe[0].sphere[2].dual() );
           auto cb = Contact( ca, sphere );
           auto cd = Contact( cyc.tframe[2].pos(), cyc.tframe[2].sphere[2].dual() );
         ...
         {
           bool bFlipX = true; bool bFlipY = false;
           auto ca = Contact( cyc.tframe[3].pos(), cyc.tframe[2].sphere[2].dual() );
           auto cd = Contact( ca, sphere );
           auto cb = Contact( cyc.tframe[2].pos(), cyc.tframe[2].sphere[2].dual() );
         ...
         case 3:
         {
           ...
           bFlipY = true;
           auto cb = Contact( cyc.tframe[0].pos(), cyc.tframe[0].sphere[2].dual() );
           auto ca = Contact( cb, sphere );
           auto cc = Contact( cyc.tframe[3].pos(), cyc.tframe[2].sphere[2].dual() );
...
struct CircularNet {
  ...
  void addAt(const DualSphere& sphere, int idx, int edge, bool bCorner=false, bool bNeg=false){
    ...
    switch( edge ){
      ...
      {
        bool bFlipX = false; bool bFlipY = true;
        auto ca = Contact( cyc.tframe[1].pos(), cyc.tframe[0].sphere[2].undual() );
        auto cb = Contact( ca, sphere );
        auto cd = Contact( cyc.tframe[2].pos(), cyc.tframe[2].sphere[2].undual() );
        auto cc = Contact( cd, sphere );
        CyclideQuad c(ca.point,cb.point,cc.point,cd.point);
        c.calcFrame( cyc.tframe[1],  bFlipX,bFlipY,bCorner,bNeg,0);
        c.log();
...
struct CyclidicVolume {
   ...
   CyclidicVolume( const SixSphere& o ) : mOrigin(o) {
     set();
   }

   /// Set origin and parallel curvature offsets
   CyclidicVolume& origin( const SixSphere& s, float tzx=0, float tzy=0, float txy=0) {
      mOrigin = s;
      set(tzx,tzy,txy);
      return *this;
   }
   ...
   void set(float tzx=0.f, float tzy=0.f, float txy=0.f) {
     cyclide[0].set(mOrigin.frame, mOrigin.y(), mOrigin.xy(txy), mOrigin.x() );
     cyclide[1].set(mOrigin.z(), mOrigin.zy(tzy), mOrigin.xyz(tzx, tzy), mOrigin.zx(tzx) );      
   }

   /// Pass in starting frame, and curvatures @todo -- depecrate
   void setAbsolute( Frame f,
             float YX, float ZX,    // constant X=0 in the Y and Z directions
             ...
             float lengthX, float lengthY, float lengthZ ){ // Length of Sides
      ...
      TangentFrame tfxz = tfz.unit().zclose(Z1X, mOrigin.frame.pos(), tfx.pos());
      TangentFrame tfyz = tfz.unit().zclose(Z1Y, mOrigin.frame.pos(), tfy.pos());
      TangentFrame tfxy = tfx.unit().xclose(X1Y, mOrigin.frame.pos(), tfy.pos());
   ...
   Pair zlog(float u) {
      ...
      return Gen::log( Gen::ratio(sph1,sph2, true), bAlt);//cyclide[0].altWU() || cyclide[0].altWV() );
   }
   ...
   /// and letters are vertices (back, clockwise, then front, clockwise)
    ...
    CyclidicVolume attach(int edge, float da, float db, float dc, float dd, float dist = 3){
      CyclidicVolume cv;
      //.......... a  d  b  e  c   h   f   g
      ...
      switch(edge){
        case 0:{
          ...
          //attach to right
          float dyx  = da; float dzx = db;
          float dz1x = dc; float dx1y = -dd;

          p = cyclide[0][3]; p.flip(0);
          x = p.xbend( dyx, dzx, dist);
          y = cyclide[0][2]; y.flip(0);
          z = cyclide[1][3]; z.flip(0);
          yz = cyclide[1][2]; yz.flip(0);
        ...
        case 1:{
          ...
          p = cyclide[0][1]; p.flip(1);
          x = cyclide[0][2]; x.flip(1);
          z = cyclide[1][1]; z.flip(1);
          xz = cyclide[1][2]; xz.flip(1);
