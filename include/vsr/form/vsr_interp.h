...
class Interp {
  ...
  */
  ...
  template <typename T>
  static T volume(T a, T b, T c, T d, T e, T f, T g, T h, double u, double v,
                  double w);
  ...
  //            template<typename T>
  //            static vector<double> svals(T a, T b, T c, T d, double u, double
  //            v);
  //
  //            template<typename T>
  //            static vector<double> svals(T a, T b, T c, T d, double u, double
...
inline T Interp ::quadric(T *cp, int num, double t, bool closed) {
  ...
  if ((rem != 0) && (it == fn)) {
    return closed ? linear(cp[gt], cp[0], ct * 2)
                  : linear(cp[gt], cp[gt + 1], ct * 2);
  } else {
    return closed ? quadric(cp[gt], cp[gt + 1],
                            (it == fn - 1) ? cp[0] : cp[gt + 2], ct)
                  : quadric(cp[gt], cp[gt + 1], cp[gt + 2], ct);
  }
}
...
//    template<>
//    inline Frame Interp :: surface<Frame>(Frame a, Frame b, Frame c, Frame d,
//    double u, double v){
...
template <typename T>
inline T Interp ::volume(T a, T b, T c, T d, T e, T f, T g, T h, double u,
                         double v, double w) {
