...
public:
  ...
  Bennett(VSR_PRECISION theta, VSR_PRECISION lengthA, VSR_PRECISION lengthB)
      : Chain("RRRR"), mLengthA(lengthA), mLengthB(lengthB), mTheta(theta),
        mPhase(0) {
    init();
  }

  Bennett(VSR_PRECISION theta, VSR_PRECISION lengthA, VSR_PRECISION lengthB,
          VSR_PRECISION offset)
      : Chain("RRRR"), mLengthA(lengthA), mLengthB(lengthB), mOffsetA(offset),
        mOffsetB(offset), mTheta(theta), mPhase(0) {
    init();
  }

  void set(VSR_PRECISION theta, VSR_PRECISION lengthA, VSR_PRECISION lengthB,
           VSR_PRECISION offset = 0.0) {
    mTheta = theta;
    mLengthA = lengthA;
    mLengthB = lengthB;
    mOffsetA = offset;
  ...
  */
  Circle circleMeet() { return (nextSphere(1) ^ prevSphere(3)).undual(); }

  Pair pairMeet() { return (nextPlane(1) ^ circleMeet().dual()).dual(); }

  Circle orbit() { return (nextPlane(3) ^ prevSphere(0)).undual(); }
  ...
  Bennett &operator()(VSR_PRECISION amt) {
    ...
    auto dualMeet = nga::Round::dls_pnt(mFrame[1].pos(), mLengthB) ^
                    nga::Round::dls_pnt(mFrame[3].pos(), mLengthA);
   ...
   */
  Bennett linkRatio(VSR_PRECISION th, VSR_PRECISION a = .5,
                    VSR_PRECISION b = .5, VSR_PRECISION la = 0,
                    VSR_PRECISION lb = 0) {
    ...
    if (la == 0)
      la = mLengthA; // else la = mLengthA;
    if (lb == 0)
      lb = mLengthB; // else lb *= mLengthB;
  ...
  /*!
   *  \brief  A linked Bennett mechanism at joint N determined by ratio of
   * original We first create a sublinkage inside the first, and then use the
   * [2] frame to set the base of our resulting linkage.
   *
   ...
   */
  Bennett linkAt(unsigned int N = 2, VSR_PRECISION th = 1, VSR_PRECISION a = .5,
                 VSR_PRECISION b = .5, VSR_PRECISION la = 0,
                 VSR_PRECISION lb = 0) {
    ...
    Bennett b2(mTheta * th, mLengthA * a, mLengthB * b);
    // set baseframe of new linkage to nth frame and undo local in-socket
    // transformation
    b2.baseFrame() = Frame(mFrame[N].mot() * !mJoint[N].rot());
    // set phase of new linkage mechanism from nth joint's rotation
    b2(bSwitch ? -nga::Gen::iphi(mJoint[N].rot())
               : nga::Gen::iphi(mJoint[N].rot()));
    ...
    return b3(bSwitch ? -nga::Gen::iphi(b2.joint(2).rot())
                      : nga::Gen::iphi(b2.joint(2).rot()));
  }

  // alt
  Bennett linkAt_(unsigned int N, VSR_PRECISION th = 1, VSR_PRECISION a = .5,
                  VSR_PRECISION b = .5) {
    ...
    b2.baseFrame() = Frame(mFrame[N].mot() * !mJoint[N].rot());
    // set phase of new linkage mechanism from nth joint's rotation
    return b2(bSwitch ? -nga::Gen::iphi(mJoint[N].rot())
                      : nga::Gen::iphi(mJoint[N].rot()));
  }
};
  ...
  void operator()() {
    ...
    for (int i = 0; i < mChainA.num(); i += 2) {
      ...
      if (isLegit) {
        ...
        if (i < mChainA.num() - 2) {
          mChainA[i + 2].pos() = Round::null(
              mChainA[i].vec() + ((mChainA[i + 1].vec() - mChainA[i].vec()) *
                                  (1 + ((flip) ? b : a))));
          mChainB[i + 2].pos() = Round::null(
              mChainB[i].vec() + ((mChainB[i + 1].vec() - mChainB[i].vec()) *
                                  (1 + ((flip) ? a : b))));
        }
