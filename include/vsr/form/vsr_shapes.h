...
struct Shape
{
  ...
  static vector<Vertex> Cylinder (int w = 20, int h = 20, float radius = 1.0,
                                  float height = 2.0)
  {
        ...
        for (int i = 0; i < stacks; ++i)
          {
            ...
            int d =
              (j < slices - 1) ? c + stacks : (i < stacks - 1) ? i + 1 : 0;
        ...
        for (int j = 0; j < num; ++j)
          {
            ...
            m.add (v[0], v[1], v[2]);
            m[m.num () - 1].Col.set ((1 + sin (pt)) / 2.0, .2,
                                     (1 + cos (pt)) / 2.0, 1);
          }
      }
        ...
        for (int j = 0; j < num; ++j)
          {
            ...
            Vec3f va =
              (m[b].Pos - m[c].Pos).cross (m[d].Pos - m[e].Pos).unit ();
            m[a].Norm = va;
          }
      }
