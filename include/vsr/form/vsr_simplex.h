...
struct Simplex
{
  ...
  void doRot ()
  {
    ...
    auto rot = nga::Gen::ratio (doBary ().unit (), vec);
    ...
    Vec tvec;
    tvec[N - 1] = 1;
    auto nrot = nga::Gen::ratio (verts.back ().unit (), tvec);
  ...
  void spin (const Biv &biv)
  {
    for (auto &i : verts)
      {
        i = i.sp (nga::Gen::rot (biv));
      }
  }

  void spin (int which, float amt = 1.0) { spin (biv (which, amt)); }
  ...
  vector<NEVec<3>> project (float dist, bool bOrtho = false) const
  {
    ...
    for (auto i : verts)
      {
        auto tmp = nga::Proj<N>::Call (dist, i);
