...
class Conic
{
 ...
 public:
  /**
    ...
    */
  template <class A>
  static GAPnt<A> Transform (const GAVec<A> &vec, const GAVec<A> &v,
                             VSR_PRECISION alpha)
  {
  ...
  /**
    ...
    */
  template <class A>
  static GAPnt<A> Transform (const GAPnt<A> &p, const GAVec<A> &v,
                             VSR_PRECISION alpha)
  {
  ...
  /**
    * @brief Transform a vsr::cga::Point relative to center
    *
    * @param veca  vsr::cga::Vec position to transform
    * @param vecb  vsr::cga::Vec direction
    * @param alpha VSR_PRECISION eccentricity
    ...
    */
  template <class A>
  static GAPnt<A> Transform (const GAPnt<A> &p, const GAPnt<A> &c,
                             const GAVec<A> &v, VSR_PRECISION alpha)
  {
    GAPnt<A> rp = (GAVec<A>(p) - GAVec<A>(c)).null();
    VSR_PRECISION talpha = (rp <= v)[0] * alpha;
    GAPnt<A> p2 = rp + GAOri<A> (1) * talpha;  // * beta
    auto wt = p2 <= GAInf<A> (1);
    auto rp2 = nga::Round::loc (p2 / -wt);
    return (GAVec<A>(c) + GAVec<A>(rp2)).null();
  }
  ...
  /**
    ...
    */
  template <class A>
  static GAPnt<A> ITransform (const GAPnt<A> &p, const GAVec<A> &v,
                              VSR_PRECISION alpha)
  {
    VSR_PRECISION talpha = (p <= v)[0] * alpha;
    GAPnt<A> p2 = p - GAOri<A> (1) * talpha;  // / beta
  ...
  /**
    * @brief Inverse Transform of a vsr::cga::Point relative to a center
    *
    * @param veca  vsr::cga::Vec position to transform
    * @param vecb  vsr::cga::Vec direction
    * @param alpha VSR_PRECISION eccentricity
    ...
    */
  template <class A>
  static GAPnt<A> ITransform (const GAPnt<A> &p, const GAPnt<A> &c, const GAVec<A> &v,
                              VSR_PRECISION alpha)
  {
    GAPnt<A> rp = (GAVec<A>(p) - GAVec<A>(c)).null();
    VSR_PRECISION talpha = (rp <= v)[0] * alpha;
  ...
  /**
    * @brief Transform a vsr::cga::Line
    *
    ...
    */
  template <class A>
  static GALin<A> Transform (const GALin<A> &line, const GAVec<A> &v,
                             VSR_PRECISION alpha)
  {
    GAPnt<A> pa = nga::Flat::loc (line, GAPnt<A> (GAOri<A> (1)), false).null ();
    GAPnt<A> tpa = Transform (pa, v, alpha);
    GAPnt<A> pb =
      pa.trs (-nga::Flat::dir (line).template copy<GAVec<A>> ().unit ());
  ...
  /**
    ...
    */
  template <class A>
  static GALin<A> ITransform (const GALin<A> &line, const GAVec<A> &v,
                              VSR_PRECISION alpha)
  {
    GAPnt<A> pa = nga::Flat::loc (line, GAPnt<A> (GAOri<A> (1)), false).null ();
  ...
  template <class A>
  static GADll<A> Transform (const GADll<A> &line, const GAVec<A> &v,
                             VSR_PRECISION alpha)
  {
    GAPnt<A> pa = nga::Flat::loc (line, GAPnt<A> (GAOri<A> (1)), true).null ();
    GAPnt<A> tpa = Transform (pa, v, alpha);
    GAPnt<A> pb = pa.trs (
      -nga::Flat::dir (line.dual ()).template copy<GAVec<A>> ().unit ());
    GAPnt<A> tpb = Transform (pb, v, alpha);
  ...
  template <class A>
  static GADll<A> ITransform (const GADll<A> &line, const GAVec<A> &v,
                              VSR_PRECISION alpha)
  {
    GAPnt<A> pa = nga::Flat::loc (line, GAPnt<A> (GAOri<A> (1)), true).null ();
    GAPnt<A> ipa = ITransform (pa, v, alpha);
  ...
  template <class A>
  static GAPln<A> Transform (const GAPln<A> &plane, const GAVec<A> &v,
                             VSR_PRECISION alpha)
  {
    //get points to apply aipoint to
    GAPnt<A> pa =
      nga::Flat::loc (plane, GAPnt<A> (GAOri<A> (1)), false).null ();
  ...
  template <class A>
  static GAPln<A> ITransform (const GAPln<A> &plane, const GAVec<A> &v,
                              VSR_PRECISION alpha)
  {
    //get points to apply aipoint to
    GAPnt<A> pa =
  ...
  template <class A>
  static GADlp<A> Transform (const GADlp<A> &plane, const GAVec<A> &v,
                             VSR_PRECISION alpha)
  {
    GAPnt<A> pa = nga::Flat::loc (plane, GAPnt<A> (GAOri<A> (1)), true).null ();
  ...
  template <class A>
  static GADlp<A> ITransform (const GADlp<A> &plane, const GAVec<A> &v,
                              VSR_PRECISION alpha)
  {
    GAPnt<A> pa = nga::Flat::loc (plane, GAPnt<A> (GAOri<A> (1)), true).null ();
    auto ca = (nga::Round::dls (pa, 1) ^ plane).dual ();
