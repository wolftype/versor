 ...
 *
 *    Description:  tangent frames, contact methods, and coordinate surface
 six-spheres in vsr::cga
...
struct Tops {
  ...
  static DualSphere Surface(const Pair &tv) { return (Inf(TMP_PSIGN) <= tv); }

  // Generate a Surface, iven a tangent and a curvature. This will be a sphere.
  // Contract Infinity with the tangent to make a plane and then multiply by (1+k*tangent)
  ...
  static DualSphere Normalize(const DualSphere &s) {
    ...
    return ((is_flat) ? DualSphere(s[0], s[1], s[2], 0, s[4]) : s / s[3]) *
           flip;
  }

  // Gives unit vector direction of Tangennt element
  static Vec Unit(const Pair &tv) { return -Round::dir(tv).copy<Vec>().unit(); }
  ...
  template <typename R> static Point Xf(const Point &p, const R &xf) {
  ...
  static Point Xf(const Point &p, const Pair &genU, const Pair &genV,
                  VSR_PRECISION amtU, VSR_PRECISION amtV) {
    return Xf(p, Gen::bst(genU * amtU) + Gen::bst(genV * amtV));
  }

  // For a Is tangent on a sphere, is it pointing to the positive side the sphere?
  static bool RelativeSense(const DualSphere &s, const Pair &p){
    return (s <= p)[3] > FPERROR;
  }

  // Is tangent Pair inside the sphere?
  static bool RelativePosition(const DualSphere &s, const Pair &p){
    return (Round::location(p) <= s)[0] > FPERROR;
  }
...
TFrame_ {
  ...
  DualSphere &suv() { return s[(int)TCS::uv]; }
  DualSphere &suw() { return s[(int)TCS::uw]; }
  DualSphere &svu() { return s[(int)TCS::vu]; }
  DualSphere &svw() { return s[(int)TCS::vw]; }
  DualSphere &swu() { return s[(int)TCS::wu]; }
  DualSphere &swv() { return s[(int)TCS::wv]; }
  ...
  // unused? perhaps change to addSurface and actually add one
  ...
  // Pass in tangent (pair) and spheres (surfaces). Spheres are encoded already
  Pair gen(const TFrame_ &f, int tidx, int sidx) {
    return Tops::CalcGen(t[tidx], f.t[tidx], s[sidx], f.s[sidx]);
  }

  // Only need single idx specifying the direction of movement
  // TCS determines the direction of movement and the surface to use.
...
struct TFrameBox {
    ...
    TFrame_ &u() { return tf[(int)CRD::u]; }
    TFrame_ &v() { return tf[(int)CRD::v]; }
    TFrame_ &w() { return tf[(int)CRD::w]; }
    TFrame_ &uv() { return tf[(int)CRD::uv]; }
    TFrame_ &uw() { return tf[(int)CRD::uw]; }
    TFrame_ &vw() { return tf[(int)CRD::vw]; }
    TFrame_ &uvw() { return tf[(int)CRD::uvw]; }

    Pair &duvw0() { return gen[(int)GEN::uvw0]; }
    Pair &duvw1() { return gen[(int)GEN::uvw1]; }
    Pair &duwv0() { return gen[(int)GEN::uwv0]; }
    Pair &duwv1() { return gen[(int)GEN::uwv1]; }
    Pair &dvuw0() { return gen[(int)GEN::vuw0]; }
    Pair &dvuw1() { return gen[(int)GEN::vuw1]; }
    Pair &dvwu0() { return gen[(int)GEN::vwu0]; }
    Pair &dvwu1() { return gen[(int)GEN::vwu1]; }
    Pair &dwvu0() { return gen[(int)GEN::wvu0]; }
    Pair &dwvu1() { return gen[(int)GEN::wvu1]; }
    Pair &dwuv0() { return gen[(int)GEN::wuv0]; }
    Pair &dwuv1() { return gen[(int)GEN::wuv1]; }

    Boost ruvw0() { return Gen::bst (duvw0()); }
    Boost ruvw1() { return Gen::bst (duvw1()); }
    Boost ruwv0() { return Gen::bst (duwv0()); }
    Boost ruwv1() { return Gen::bst (duwv1()); }
    Boost rvuw0() { return Gen::bst (dvuw0()); }
    Boost rvuw1() { return Gen::bst (dvuw1()); }
    Boost rvwu0() { return Gen::bst (dvwu0()); }
    Boost rvwu1() { return Gen::bst (dvwu1()); }
    Boost rwvu0() { return Gen::bst (dwvu0()); }
    Boost rwvu1() { return Gen::bst (dwvu1()); }
    ...
    void buildBackwards(const Frame &f){
        ...
        uw() = TFrame_(puw(), uvw(), TDIR::v);
        uv() = TFrame_(puv(), uvw(), TDIR::w);

        u() = TFrame_(pu(), uv(), TDIR::v);
        u().addSurfaces(uw(), TDIR::w);

        v() = TFrame_(pv(), vw(), TDIR::w);
        v().addSurfaces(uv(), TDIR::u);

        w() = TFrame_(pw(), uw(), TDIR::u);
        w().addSurfaces(vw(), TDIR::v);

        o() = TFrame_(po(), u(), TDIR::u);
        o().addSurfaces(v(), TDIR::v);
        o().addSurfaces(w(), TDIR::w);
    ...
    void calcGen(){
        ...
        // Left Face
        dwvu0() = o().gen(w(), TCS::wv);
        dvwu0() = o().gen(v(), TCS::vw);

        // Bottom Face
        dwuv0() = o().gen(w(), TCS::wu);
        duwv0() = o().gen(u(), TCS::uw);

        // Right Face
        dwvu1() = u().gen(uw(), TCS::wv);
        dvwu1() = u().gen(uv(), TCS::vw);

        // Top Face
        dwuv1() = v().gen(vw(), TCS::wu);
        duwv1() = v().gen(uv(), TCS::uw);

        // Back Face
        duvw1() = w().gen(uw(), TCS::uv);
        dvuw1() = w().gen(vw(), TCS::vu);

    }

    std::array<Pair, 3> calcTensor(float z){
        ...
        // u10 = u00 moved across the bottom
        Pair u10 = Tops::NormalizePair( u00.spin( ruwv0() ) );

        // v01 = v00 moved up the left side
        Pair v01 = Tops::NormalizePair( v00.spin( rvwu0() ) );

        // u11 = u10 moved up the right side
        Pair u11 = Tops::NormalizePair( u10.spin( rvwu1() ) );

        // v11 = v01 moved across the top side
        Pair v11 = Tops::NormalizePair( v01.spin( ruwv1() ) );

        DualSphere su0 = Tops::Normalize(v().svw() <= u00);
        DualSphere sv0 = Tops::Normalize(u().suw() <= v00);

        DualSphere su1 = Tops::Normalize(sv0 <= u11);
        DualSphere sv1 = Tops::Normalize(su0 <= v11);

        Pair logU = Tops::CalcGen(u00, u10, su0, su1);
...
struct TFrame {
  ...
  DualSphere usurf(float ku) {
    ...
    return Tops::Surface(
        tu, ku); // Normalize( (Inf (TMP_PSIGN) <= tu) * ((tu * ku) + 1));
  }
  ...
  DualSphere vsurf(float kv) {
    ...
    return Tops::Surface(
        tv, kv); // Normalize( (Inf (TMP_PSIGN) <= tv) * ((tv * kv) + 1));
  }
  ...
  DualSphere wsurf(float kw) {
    ...
    return Tops::Surface(
        tw, kw); // Normalize ((Inf (TMP_PSIGN) <= tw) * ((tw * kw) + 1));
  }
  ...
  TFrame xf(const R &k, bool uflip, bool vflip, bool wflip) {
    ...
    return TFrame(Tops::Xf(tu, k, uflip), Tops::Xf(tv, k, vflip),
                  Tops::Xf(tw, k, wflip));
  }
  ...
  // not used
  //  static Pair CalcGen2 (const Point& p, const DualSphere& beg, const
  //  DualSphere&end)
  //  {
  //    auto ratio = (end/beg).tunit();
  ...
  //  static Pair CalcGen (const Pair& p1, const Pair& p2, const DualSphere&
  //  beg, const DualSphere&end)
  //  {
  //    // The Transformation Operator
  //    auto ratio = (end/beg).tunit();
  //    // Relative sense of begining tangent to its constant sphere
  ...
  //    // Relative sense of ending tangent to its constant sphere
  //    bool flipB = (end <= p2)[3] > FPERROR;
  //    // Relative location of the begining tangent to the ending constant
  //    sphere bool flipC =  ((Round::location(p1) <= end)[0] > FPERROR) ;
  //
  //    // Are relative senses at start and finish the same?
  //    bool bRelSense = flipA != flipB;
  //
  //    // Approach: Are we approaching the sphere surface from the same
  //    direction
  //    // As it's tangent?
  //    bool bRelApproach  = flipC == flipB;
  //
  ...
  Pair duv(const TFrame &tf) { return Tops::CalcGen(tu, tf.tu, suv, tf.suv); }
  // this sweeps the w direction curve "over" along u
  Pair duw(const TFrame &tf) { return Tops::CalcGen(tu, tf.tu, suw, tf.suw); }
  // this sweeps the u direction curve "up" along v
  Pair dvu(const TFrame &tf) { return Tops::CalcGen(tv, tf.tv, svu, tf.svu); }
  // this sweeps the w direction curve "up" along v
  Pair dvw(const TFrame &tf) { return Tops::CalcGen(tv, tf.tv, svw, tf.svw); }
  // this sweeps the u direction curve "in" along w
  Pair dwu(const TFrame &tf) { return Tops::CalcGen(tw, tf.tw, swu, tf.swu); }
  // this sweeps the v direction curve "in" along w
  Pair dwv(const TFrame &tf) { return Tops::CalcGen(tw, tf.tw, swv, tf.swv); }
};
...
struct TVolume {
  ...
  enum class Face { LEFT, RIGHT, BOTTOM, TOP, BACK, FRONT };

  // An array of conformal mappings to be applied to any element
  struct Mapping {

    std::vector<Con> mCon;
    unsigned int mResU, mResV, mResW;

    Mapping(unsigned int resU, unsigned int resV, unsigned int resW)
        : mResU(resU), mResV(resV), mResW(resW) {
      mCon.resize(resU * resV * resW);
    }

    Con &at(unsigned int i, unsigned int j, unsigned int k) {
      return mCon[i * mResV * mResW + j * mResW + k];
    }

    const Con &at(unsigned int i, unsigned int j, unsigned int k) const {
      return mCon[i * mResV * mResW + j * mResW + k];
    }
  };
  ...
  TVolume(const TVolume &tv, Face face, float kA = 0.0, float kB = 0.0,
          float kC = 0.0, float kD = 0.0) {
    zeroInit();
    switch (face) {
    case Face::LEFT: {
      break;
  ...
  TVolume(float kvu, float kwu, float kuv, float kwv, float kuw, float kvw,
          float kv1u, float ku1w, float kw1v, float uSpacing, float vSpacing,
          float wSpacing) {
    mK[0] = kvu;
    mK[1] = kwu;
    mK[2] = kuv;
  ...
  Coord inverseMapping(const Point &p, const Face &face) {
    auto imapval = [](const Pair &pa, const Pair &pb, const DualSphere &a,
                      const DualSphere &b) {
      return Ori(-1) <= Tops::CalcGen(pa, pb, a, b);
      ;
    };
  ...
  Point pointAt(const Coord &coord) {
    return Round::location(
        pos().spin(calcMappingAt(coord.u, coord.v, coord.w)));
  }
};
...
struct TangentFrame : public Frame {
  ...
  ///  Sphere through next point with tangents as below (needed or put in next
  ///  level?)
  Sphere sphere[3];
  ...
  TangentFrame unit() const {
    ...
    for (int idx = 0; idx < 3; ++idx) {
      ...
      tf.bitan[idx] =
          Circle(Round::direction(bitan[idx]).copy<Biv>().tunit().copy<Tnb>())
              .translate(pos());
  ...
  /// Generate Boost Relative to x and z TangentVector (null Pair) (dx/dy +
  /// dz/dy)
  ...
  /// Given a sphere and two points, make a circle with the two points and find
  /// its intersection with sphere
...
/**
* @brief Contact provides methods for calculations with a normal tangent vector
at a point on a sphere
...
/// SixSphere Coordinate System curvatures in every direction (see Moon and
/// Spencer for coordinate system diagram)
