...
struct EGProd{
  ...
  typedef typename Fun::Type Bases;         //<-- Basis of Return Type
...
struct EOProd{
  ...
  typedef typename Fun::Type Bases;
...
struct RMGProd{
  typedef typename MGP<typename A::Bases, typename B::Bases, M>::Type InstList;      
  typedef typename Index< InstList, R>::Type Arrow;   
  static constexpr R Call(const A& a, const B& b) {
    return Arrow::template Make<R>(a, b);
  } 
