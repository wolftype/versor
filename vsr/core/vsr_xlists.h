...
struct RMetric{
  ...
  using Type = typename Cat< typename RPlus<P>::Type, typename RMinus<Q>::Type >::Type;  //metric as list

  ///Next Higher (positive)
  using up = RMetric<P+1,Q,bConformal>;
  ///Next Lower (positive)
  using down = RMetric<P-1,Q,bConformal>;
  ///Euclidean Subspace
  using euclidean = RMetric<P-1>;
  ///Conformal Mapping
  using conformal = RMetric<P+1,1,true>; 
