...
struct EGA{    
  ...
  using Pnt = Vec; //Some Templated classes, like Field, require the subdomain to have a Pnt type to fill a lattice . . .
...
struct CGAMV : public A {                 
  ...
  auto operator + (const CGAMV<DIM, B>& b) const -> CGAMV< DIM, decltype( sum( *this, B() ) ) >  {
   ...
   //IMPLEMENTATION OF METHODS BELOW ARE IN vsr_generic_op.h and vsr_cga3D_op.h
   //NOTE NEED TO BE MADE CONST!!
...
struct EGAMV : public A {
  ...
  //Make from some other dimension 
