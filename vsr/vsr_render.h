...
namespace vsr{   
  ...
  void Render(const Frame& frame, const Mat4f& mvm, Pipe& pipe, float r = 1.0, float g = 1.0, float b = 1.0, float a = 1.0 )  { 
    ...
    /* fm.mesh[0].Pos = frame.pos(); */
    /* fm.mesh[1].Pos = frame.x() * frame.scale(); */
    /* fm.mesh[2].Pos = frame.y() * frame.scale(); */
    /* fm.mesh[3].Pos = frame.z() * frame.scale(); */
  ...
  void Render(const Sph& sph, const Mat4f& mvm, Pipe& pipe, float r= 1.0, float g = 1.0, float b= 1.0, float a = 1.0 ){
    Render (sph.dual(), mvm, pipe, r,g,b,a);
  }
