...
struct MyApp : App {
  ...
  virtual void onSetup() {
    ...
    // A circle at coordinate -3,0,0
    ca = Construct::circle(-3, 0, 0, Biv::xy, 1);
    // A circle at coordinate 3,0,0
    cb = Construct::circle(3, 0, 0, Biv::xy, 1);
  }

  virtual void onDraw() {
    // twist circle b around x axis dualline
    auto tca = Construct::circle(-dist / 2.0, 0, 0, Biv::xy, 1)
                   .twist(Construct::line(1, 0, 0).dual() * -amt);
    // twist circle b around x axis dualline
    ...
    float planarity =
        (Round::carrier(tca).dual().unit() ^ Round::carrier(tcb).dual().unit())
            .wt();
    ...
    for (int i = 0; i < num; ++i) {
      ...
      // general rotation ("spin") by
      auto nc = tca.spin(Gen::con(log, t * (bPos ? 1 : -1)));

      // general conformal transformation
      // auto nnc = (tca * sin((1-t)*theta) + (tcb * sin(t*theta)) ) /
      // sin(theta);
...
//       trot = -trot;//(-b/a).runit();//-trot; //restrict to positive <R> only
//       if coplanar
//     }
//
//     auto rotone = trot + 1;  //<-- we are going to "normalize" this by polar
//     decomposition
//
