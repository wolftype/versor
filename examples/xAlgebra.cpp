...
// a euclidean algebra of 3d space (basis elements include vectors, areas,
// volumes, rotors (aka quaternions) )
//...................................<p>, field  >
...
//...................................<p,q>, field  >  ( p is number of basis
// roots of 1 and q number of roots of -1)
using sta = vsr::algebra<vsr::metric<1, 3, 0, false>, double>;

// a conformal algebra of 3d space + 2 (we add two new dimensions to the
// "signature" and set conformal flag to "true")
//...................................<p,q,conf?>  field >
...
/*-----------------------------------------------------------------------------
  ...
  can be any type of number that returns the same type under multiplication and
 addition and is commutative, that is a*b = b*a (additionally it must have
 characteristic != 2 )

  for instance, we can define a 2 dimensional geometric algebra over the complex
 numbers

 *-----------------------------------------------------------------------------*/

//  a complex number is isomorphic to a "rotor" in 2 dimensional euclidean
//  geometric algebra
template <class T>
using complex_t =
    typename vsr::algebra<vsr::metric<2, 0, 0, false>, T>::types::rotor;

// c2 is now a four-dimensioal complex number in the tensored space i.e. with
// complex arguments
using c2 = complex_t<complex_t<double> >;

/*-----------------------------------------------------------------------------

  template alias-driven "syntactic sugar" exists for common spaces such as
 euclidean and conformal. in the first line of code above we could have written:
  ...
  Also, "::types::rotor" is syntactic sugar for ::make_sum< blade<2,0>::type,
 blade<2,2>::type >) which generates a multivector type for the algebra

 *-----------------------------------------------------------------------------*/
static_assert(std::is_same<ega, vsr::euclidean<3> >::value,
              "euclidean geometric algebra of 3D space");
static_assert(std::is_same<cga, vsr::conformal<5> >::value,
              "conformal geometric algebra of 3D space");

int main() {
  static_assert(std::is_same<ega::types::vector::basis,
                             typename vsr::blade<3, 1>::type>::value,
                "ega3d vector basis");
  static_assert(std::is_same<ega::types::bivector::basis,
                             typename vsr::blade<3, 2>::type>::value,
                "ega3d bivector basis");
  ...
  printf(
      "some combinatorics: product of rotors a and b in euclidean algebra: \n");
  ega::impl::gp_arrow_t<ega::types::rotor::basis,
                        ega::types::rotor::basis>::Arrow::print();
  printf(
      "some combinatorics: product of vectors a and b in space time algebra: "
      "\n");
  sta::impl::gp_arrow_t<sta::make_grade<1>::basis,
                        sta::make_grade<1>::basis>::Arrow::print();
  printf(
      "some combinatorics: product of vectors a and b in euclidean 4d algebra: "
      "\n");
  vsr::euclidean<4>::impl::gp_arrow_t<
      sta::make_grade<1>::basis, sta::make_grade<1>::basis>::Arrow::print();
  printf(
      "some combinatorics: product of bivectors a and b in twistor algebra: "
      "\n");
  twistor::impl::gp_arrow_t<sta::make_grade<2>::basis,
                            sta::make_grade<2>::basis>::Arrow::print();

  printf(
      "some combinatorics: inner product of rotors a and b in euclidean 3D "
      "algebra: \n");
  ega::impl::ip_arrow_t<ega::types::rotor::basis,
                        ega::types::rotor::basis>::Arrow::print();

  printf(
      "some combinatorics: scalar product of rotors a and b in euclidean 3D "
      "algebra: \n");
  ega::impl::sp_arrow_t<ega::types::rotor::basis,
                        ega::types::rotor::basis>::Arrow::print();

  printf(
      "some combinatorics: inner product of twist motors a and b in conformal "
      "5D algebra: \n");
  cga::impl::ip_arrow_t<cga::types::motor::basis,
                        cga::types::motor::basis>::Arrow::print();

  printf(
      "some combinatorics: scalar product of twist motors a and b in conformal "
      "5D algebra: \n");
  cga::impl::sp_arrow_t<cga::types::motor::basis,
                        cga::types::motor::basis>::Arrow::print();
