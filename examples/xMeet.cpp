...
struct MyApp : App {
  ...
  DualSphere sphereA = sphere(-.5,0,0,1);
  DualSphere sphereB = sphere(.5,0,0,1);
  ...
  void onDraw(){
    ...
    draw(Ro::surround(meet),0,0,1,.5);  ///<-- draw surround of meet (a sphere) in blue with half alpha
   ...
   //  if ( ( (p<=sphereA)[0] > 0 ) && ( (p<=sphereB)[0] > 0 ) ) cout << "HIT" << endl;
