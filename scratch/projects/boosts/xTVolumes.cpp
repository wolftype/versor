    ...
    for (int i = 0; i < resU; ++i) {
      ...
      for (int j = 0; j < resV; ++j) {
        ...
        for (int k = 0; k < resW; ++k) {
          ...
          if (k == resW - 1) {
            auto coord =
                vol2.inverseMapping(ptFrontRight, TVolume::Face::FRONT);
            faces[6].pnts[i * resV + j] =
                vol2.pointAt(ti, bInverse ? coord.v : tj, 1.0);
          }
          // Top
          if (j == resV - 1) {
            auto coord = vol2.inverseMapping(ptTopRight, TVolume::Face::LEFT);
            faces[7].pnts[i * resW + k] =
                vol2.pointAt(ti, 1.0, bInverse ? coord.w : tk);
          }
          // Bottom
          if (j == 0) {
            auto coord =
                vol2.inverseMapping(ptBottomRight, TVolume::Face::LEFT);
            faces[8].pnts[i * resW + k] =
                vol2.pointAt(ti, 0.0, bInverse ? coord.w : tk);
          }
          // Back
          if (k == 0) {
            auto coord = vol2.inverseMapping(ptBackRight, TVolume::Face::BACK);
            faces[9].pnts[i * resV + j] =
                vol2.pointAt(ti, bInverse ? coord.v : tj, 0.0);
          }
          // Right
          if (i == resU - 1) {
            ...
            auto coordW =
                vol2.inverseMapping(ptBottomRight, TVolume::Face::LEFT);
            faces[10].pnts[j * resW + k] = vol2.pointAt(
                1.0, bInverse ? coordV.v : tj, bInverse ? coordW.w : tk);
          }
        }
      }
    }
