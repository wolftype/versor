...
struct MyApp : App
{
  ...
  void setup ()
  {
    ...
    gui (bDrawTrilinear, "bDrawTrilinear");
    ...
    for (auto &i : graph.edge ())
      {
        Vec ea = i->a () - i->b ();
        Vec eb = i->next->a () - i->a ();
        float dot = (ea.unit() <= eb.unit())[0];
        origDot.push_back (dot);
      }
  }
  ...
  void onDraw ()
  {
    ...
    auto frameAtU = [&](const Vec &coord) {
      float tu = coord[0];
      float tv = coord[1];
      float tw = coord[2];

      Boost bV0 = Gen::bst (logVu0 * tv);
      Boost bV1 = Gen::bst (logVu1 * tv);
      Boost bW0 = Gen::bst (logWu0 * tw);
      Boost bW1 = Gen::bst (logWu1 * tw);

      DualSphere u0 = tf.suw.spin (bV0);
      DualSphere u1 = u.suw.spin (bV1);
      ...
      if (bDrawTrilinear)
        {
          DrawRound (u0, 1, 0, 0, .2);
          DrawRound (u1, 1, 0, 0, .2);
        }

      return tf.xf (bU * bW0 * bV0, false, false, false);
    };
    auto frameAtV = [&](const Vec &coord) {
      float tu = coord[0];
      float tv = coord[1];
      float tw = coord[2];

      Boost bU0 = Gen::bst (logUv0 * tu);
      Boost bU1 = Gen::bst (logUv1 * tu);
      Boost bW0 = Gen::bst (logWv0 * tw);
      Boost bW1 = Gen::bst (logWv1 * tw);

      DualSphere v0 = tf.svw.spin (bU0);
      DualSphere v1 = v.svw.spin (bU1);

      Pair tlogV = Gen::log (-(v1 / v0).runit ()) / 2.0;

      Boost bV = Gen::bst (tlogV * tv);

      if (bDrawTrilinear)
        {
          DrawRound (v0, 0, 1, 0, .2);
          DrawRound (v1, 0, 1, 0, .2);
        }

      return tf.xf (bV * bW0 * bU0, false, false, false);
    };

    auto frameAtW = [&](const Vec &coord) {
      ...
      DualSphere w0 = tf.swv.spin (bU0);
      DualSphere w1 = w.swv.spin (bU1);
      ...
      if (bDrawTrilinear)
        {
          ...
          DrawRound (w0, 0, 1, 1, .2);
          DrawRound (w1, 0, 1, 1, .2);
        }
    ...
    //    if (bDrawTrilinear)
    //      {
    //        frameAt (Vec (vAmt2, .5, .5));
    //      }
    //
    auto frameAt3 = [&](const Vec &coord) {
      ...
      //Bottom Sweep
      Boost bU = Gen::bst (logUv0 * tu);
      //Back Sweep
      Boost bV = Gen::bst (logVw0 * tv);
      Con bUV = bV * bU;
      //Side Sweep
      Boost bW = Gen::bst (logWu0.spin (bUV) * tw);

      return tf.xf (bW * bV * bU, false, false, false);
    };
