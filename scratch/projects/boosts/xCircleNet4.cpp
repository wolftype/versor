...
template <>
void Renderable<CyclideQuad>::DrawImmediate (const CyclideQuad &s)
{
  for (int i = 0; i < 4; ++i)
    {
      auto tx =
        -Round::dir (s.tframe[i].bitan[0].undual ()).copy<Vec> ().unit ();
      auto ty =
        -Round::dir (s.tframe[i].bitan[1].undual ()).copy<Vec> ().unit ();
      auto tz =
        -Round::dir (s.tframe[i].bitan[2].undual ()).copy<Vec> ().unit ();
 ...
 *-----------------------------------------------------------------------------*/
template <>
void Renderable<vsr::HEGraph<MyData>, 0>::DrawImmediate (
...
struct MeshRender
{
  ...
  static void Draw (const vector<MyData> &mesh, int w, int h,
                    bool bNormals = false)
  {
...
struct CyclideDraw
{
  ...
  void draw (const CyclideQuad &cyclide, int surf = 0)
  {
    ...
    if (bDrawPlunge)
      {
        Draw (cyclide.tframe[2].sphere[2].dual () ^ cyclide.tframe[0].tan[2], 1,
              0, 1);
        Draw (cyclide.tframe[2].sphere[2].dual () ^ cyclide.tframe[1].tan[2], 1,
              0, 1);
        Draw (cyclide.tframe[1].sphere[2].dual () ^ cyclide.tframe[0].tan[2], 1,
              0, 1);
        Draw (cyclide.tframe[1].sphere[2].dual () ^ cyclide.tframe[3].tan[2], 1,
              0, 1);
      }
...
struct MyApp : App
{
  ...
  bool bDrawA, bDrawB, bDrawC, bDrawD, bDrawE, bDrawF, bDrawForward,
    bDrawNormal, bDrawEdges, bDrawCircleNet;
  bool bDrawCyclide, bDrawPlunge, bDrawSpheres, bDrawSpheresAround,
    bDrawSurface, bSmooth, bDrawNormals;
  ...
  void setup ()
  {
    gui (amt, "spinxy", -100, 1000) (amt2, "theta", -100,
                                     1000) (amt3, "height", -100,
                                            100);  //(bReset,"bReset");
    ...
    gui (bFlipLeft, "bFlipLeft");
    gui (bAddFront, "bAddFront");
    gui (bFlipFront, "bFlipFront");
    gui (bAddBottom, "bAddBottom");
    gui (bFlipBottom, "bFlipBottom");
    gui (bAddTop, "bAddTop");
    gui (bFlipTop, "bFlipTop");
    gui (bFlipLog, "bFlipLog");

    gui (bDrawA, "circles") (bDrawB, "surface") (bDrawC, "const_coord") (
      bDrawD, "edges") (bDrawE, "z");  //(bDrawF)(bDrawNormal);
  ...
  void onDraw ()
  {
    ...
    auto ptA = Construct::point (fa.bound (), Vec::z);
    Draw (ptA, 1, 0, 0);
    auto ptB = Construct::point (fb.bound (), Vec::z);
    auto sphA =
      fa.bound ().spin (Gen::bst (Pair (Tnv (0, 0, 1)).trs (ptA) * flatness));
    auto sphB =
      fb.bound ().spin (Gen::bst (Pair (Tnv (0, 0, 1)).trs (ptB) * flatness));
    ...
    Circle cir_top =
      (sphA ^ Dlp (0, 1, 0, amt3).rotate (Biv::xy * amt)).dual ();
    ...
    Rot trotB =
      Gen::ratio (Vec::x.spin (trotA),
                  Vec (Op::project (Vec (cd.point - ca.point),
                                    Biv::xy.spin (trotA)))
                    .unit ());  // -Round::dir(tan1a).copy<Vec>().unit());
    Rot trotC = Gen::rot (
      (Vec::x.spin (trotB * trotA) ^ Vec::y.spin (trotB * trotA)) * xfu);
        ...
        if (bAddTop)
          {
            ...
            if (bAddLeft)
              net.addAt (sphLeft, 0, 3, true);
            if (bAddBottom)
              net.addAt (sphBottom, 0, 2, true);
            if (bAddFront)
              net.addAt (sphFront, 2, 0, true, bFlipFront);

            //Draw these sphere coordinates
            if (bDrawSpheresAround)
              {
                if (bAddLeft)
                  Draw (sphLeft, 0, 1, 1, alpha);
                if (bAddBottom)
                  Draw (sphBottom, 0, 1, 1, alpha);
                if (bAddTop)
                  Draw (sphTop, 0, 1, 1, alpha);
                if (bAddFront)
                  Draw (sphFront, 0, 1, 1, alpha);
              }
    ...
    if (bDrawSpheres)
      {
        ...
        if (bSmooth)
          {
            if (bAddBottom)
              Draw (sphD, 0, 1, 1, alpha);
            if (bAddTop)
              Draw (sphE, 0, 1, 1, alpha);
            if (bAddFront)
              Draw (sphF, 0, 1, 1, alpha);
          }
      }
    ...
    for (int i = 0; i < 4; ++i)
      {
        ...
        auto tx = -Round::dir (cyclide.tframe[i].bitan[0].undual ())
                     .copy<Vec> ()
                     .unit ();
        auto ty = -Round::dir (cyclide.tframe[i].bitan[1].undual ())
                     .copy<Vec> ()
                     .unit ();
        auto tz = -Round::dir (cyclide.tframe[i].bitan[2].undual ())
                     .copy<Vec> ()
                     .unit ();
      }
        ...
        if (bDrawD)
          {
            if (i & 1)
              {
                Draw (cyclide.tframe[i].bitan[0], 1, 0, 0);
              }
            else
              {
                Draw (cyclide.tframe[i].bitan[1], 0, 1, 0);
              }
          }
