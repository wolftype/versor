 ...
 *
 *    Description:  two osculating spheres, two points, and a rotation for blending surfaces
 *
 *        Version:  1.0
...
struct MyApp : App
{
  ...
  bool bDrawA, bDrawB, bDrawC, bDrawD, bDrawE, bDrawF, bDrawForward,
    bDrawNormal, bDrawEdges;

  using MyData = VertexPosition;  // (vec3f pos and vec3f norm)
  ...
  void onDraw ()
  {
    ...
    Rot trotB = Gen::ratio (Vec::x.spin (trotA),
                            -Round::dir (tan1a).copy<Vec> ().unit ());
    Rot trotC = Gen::rot (
            ...
            for (int j = 0; j <= height; j++)
              {
                ...
                auto tan = cyclide.apply (cyclide.tframe[0].tz (), tu, tv);

                int idx = i * (height + 1) + j;
                mesh[idx].Pos.set (np);
                mesh[idx].Norm.set (-Round::dir (tan).copy<Vec> ().unit ());
                // Draw(np,0,1,1);
              }
          }
    ...
    for (int i = 0; i < 4; ++i)
      {
        ...
        auto tx =
          -Round::dir (cyclide.tframe[i].bitan[0].dual ()).copy<Vec> ().unit ();
        auto ty =
          -Round::dir (cyclide.tframe[i].bitan[1].dual ()).copy<Vec> ().unit ();
        auto tz =
          -Round::dir (cyclide.tframe[i].bitan[2].dual ()).copy<Vec> ().unit ();
