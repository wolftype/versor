 ...
 * =====================================================================================
 *
 *       Filename:  xDifferential4.cpp
 *
 *        Version:  1.0
 *        Created:  05/01/2024 16:39:32
 *       Revision:  none
 *       Compiler:  gcc
 *
 *       Goal: Find connections between DEC and DGC
 *
...
struct VertexData
{
  ...
  Point pnt;                    //position
  Vec normal = Vec (0, 0, 0);   //normal (volume or area gradient)
  Vec dn = Vec (0, 0, 0);       //differential of normal
  Rot drot = Rot (0, 0, 0, 0);  //total derivative
  ...
  vector<Simplicial2> simplex;  //simplicial domain around the vertex
};
...
struct MyApp : App
{
  ...
  void onDrawGui()
  {
    ...
    gui (bDivideByArea,
         "bDivideByArea");         //<-- use vector derivatives then exterior
  ...
  void onAnimate ()
  {
    ...
    //1. calculate reciprocal simplices in neighborhood and normal at v
    //2. (use edge node as root?)
    for (auto &i : graph.node ())
      {
        //vertex data, reset to zero
        auto &v = i->data ();
        v.reset ();
        ...
        for (int j = 0; j < e.size (); ++j)
          {
            ...
            // The triangle, counter clockwise from node 
            ...
            deficit -= fabs (
              acos (((Vec (c - a).unit ()) <= ((Vec (b - a).unit ())))[0]));
          }
    ...
    for (auto &i : graph.node ())
      {
        ...
        v.kg = bUseCotan
                 ? v.deficit
                 : (bFullDerivative ? tmp2 : bExteriorDerivative ? tmp1 : tmp0);
