...
struct MyApp : App {
    ...
    par[i] = Par(Tnv(0,1,0)).trs( pos[i] );
     ...
     for (int i = 0; i < numDipoles; ++i){
      Par tpar;
        ...
        if (i != j){
        VSR_PRECISION dist = Ro::sqd( pos[i], pos[j] ) + 1.;
  ...
  for (int i = 0; i< f.num();++i){
    ...
    for (int j = 0; j < numDipoles; ++j){
      VSR_PRECISION dist = Ro::sqd( f.grid(i), pos[j] ) + 1.;
      tpar +=  par[j] *  (amt /(dist) );
    }
  ...
  glBegin(GL_LINES);
  for (int i = 0; i< f.num(); ++i) {
    VSR_PRECISION aamt = fabs( ( f[i] <= Vec::x )[0] );
  ...
  Biv br = Biv::xy * PIOVERTWO ;
    ...
    for (int j = 0; j < pRes; ++j){
      ...
      // Pnt pvb = Point ( nvb );
      //        for (int j = 0; j < numDipoles; ++j){
      //   VSR_PRECISION distA = Ro::sqd( pva, pos[j] ) + 1.;
      //   VSR_PRECISION distB = Ro::sqd( pvb, pos[j] ) + 1.;
