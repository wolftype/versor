...
Pnt rframe (const Pnt &pa, const Pnt &pb)
{
  ...
  VSR_PRECISION sqnorm =
    norm * norm;  //this is just the wt() (self dot product)
...
struct MyApp : App
{
  ...
  void setup ()
  {
    ...
    gui (bNormalize, "normalize") (bAreaNormal, "areaNormal");
    gui (bAreaWeight, "areaWeight");
  }
  ...
  virtual void onDraw ()
  {
    ...
    for (int i = 0; i < num; ++i)
      {
        float t = (float) i / num;
        nbr.push_back (Round::null (
          Vec::x.rot (Biv::xz * (PI + PIOVERTWO * t))));  // + PIOVERTWO * t
        Draw (nbr[i], 1, 0, 0);
      }
    ...
    for (int i = 0; i < num; i++)
      {
        ...
        trp = bNormalize ? Round::cen (trp)
                         : trp;  // /= ( bNormalize ? trp.norm() : 1);
        rp.push_back (trp);
        if (bDrawRecip)
          Draw (trp, 0, 1, 1, .4);  //+ Inf(amtA)
      }
