...
Grt Gen::rot (const Dll &d)
{
  return nga::Gen::rot (d);
}

Grt Gen::rotor (const Dll &d)
{
  return nga::Gen::rot (d);
}

Dll Gen::log (const Grt &g)
{
...
Tsd Gen::dilator (const Pnt &p, VSR_PRECISION t)
{
  return nga::Gen::dil (p, t);
}
...
/*! theta of rotation @todo eliminate code duplication redundancy with atanh2 function */
...
/*!
       ...
       */
Pair Construct::pair (VSR_PRECISION x, VSR_PRECISION y, VSR_PRECISION z,
                      Vec vec, VSR_PRECISION r)
{
...
{
  return Round::produce (Round::dls (r * -1, 0, 0, 0),
                         Op::dle (v));  //a ^ b ^ c;
}
