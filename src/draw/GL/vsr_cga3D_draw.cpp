...
template <>
void Renderable<cga::DualPlane, 0>::DrawImmediate(const cga::DualPlane &s) {
...
template <>
void Renderable<cga::Circle, 0>::DrawImmediate(const cga::Circle &s) {
  // VSR_PRECISION rad = nga::Round::rad( s );
  VSR_PRECISION size = nga::Round::size(s, false);

  if (size > 10000)
    Renderable<cga::Line, 0>::DrawImmediate(cga::Line(s));
  else {
    bool im = size > 0 ? 1 : 0;
...
template <> void Renderable<cga::Point, 0>::DrawImmediate(const cga::Point &s) {
  ...
  if (fabs(ta) > FPERROR) {
    ...
    //          (real) ? gfx::Glyph::SolidSphere(t, 5+ floor(t*30),
    //          5+floor(t*30)) : Glyph::Sphere(t);
    (real) ? gfx::Glyph::SolidSphere(t, 5 + floor(30), 5 + floor(30))
           : Glyph::Sphere(t);
  } else {
    // printf("NOOOOOO\n");
    gfx::Glyph::Point(s);
...
template <>
void Renderable<cga::Sphere, 0>::DrawImmediate(const cga::Sphere &s) {
...
template <>
void Renderable<cga::FlatPoint, 0>::DrawImmediate(const cga::FlatPoint &s) {
...
template <> void Renderable<cga::Pair, 1>::DrawImmediate(const cga::Pair &s) {
  ...
  if (fabs(size) < FPERROR) {
    ...
    if (fabs(ta) > FPERROR) {
      ...
      glPushMatrix();
      gfx::GL::translate(p1.begin()); //(p1[0], p1[1], p1[2]);
      gfx::Glyph::SolidSphere(t, 5 + floor(t * 30),
                              5 + floor(t * 30)); // : gfx::Glyph::Sphere(t);
      glPopMatrix();
...
template <>
void Renderable<cga::DualLine, 0>::DrawImmediate(const cga::DualLine &s) {
...
//  template<> void Renderable< cga:: >::DrawImmediateB( const Field< cga::Vec>&
//  f){
//    for (int i = 0; i < f.num(); ++i){
...
template <>
void Renderable<Field<cga::Point>, 0>::DrawImmediate(
    const Field<cga::Point> &f) {
  for (int i = 0; i < f.num(); ++i) {
    // Draw( f[i] );
    render::begin();
    render::draw(f[i]);
...
//   *  Specific Strategies
//   *-----------------------------------------------------------------------------*/
//  template<>  void Renderable<Field<cga::Sca>>::DrawImmediate( const
//  Field<cga::Sca>& f){
//    static Pnt p = Pnt();
//    for (int i = 0; i < f.num(); ++i){
...
//  }
//
//  template<>  void Renderable<Field<cga::Vec>>::DrawImmediate( const
//  Field<cga::Vec>& f){
//    for (int i = 0; i < f.num(); ++i){
...
//
//
//  template<>  void Renderable<Field<cga::Tnv>>::DrawImmediate( const
//  Field<cga::Tnv>& f){
//    for (int i = 0; i < f.num(); ++i){
//      render::drawAt( f[i], f.grid(i) );
//    }
