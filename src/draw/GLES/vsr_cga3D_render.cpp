...
template <> MeshBuffer<Circle>::MeshBuffer() { mMBO.push_back(Mesh::Circle()); }
...
template <> MeshBuffer<Biv>::MeshBuffer() { mMBO.push_back(Mesh::Circle()); }
...
template <> void MeshBuffer<Field<Vec>>::Add(const Field<Vec> &f) {
  ...
  fs << &f;
  mMBOmap[fs.str()] =
      MBO(Mesh::Points2(f.gridPtr(), f.dataPtr(), f.num()).mode(GL::L),
          GL::DYNAMIC);
}
...
template <> void Renderable<Frame>::Draw(const Frame &frame, GFXSceneNode *re) {
  Renderable<MBO>::Draw(
      Get()[0], vsr::xf::mat(frame.rot(), frame.vec(), frame.scale()), re);
}
/*!
 *  Render a Circle
 */
 ...
 */
template <>
void Renderable<vector<Cir>>::Draw(const vector<Cir> &cir, GFXSceneNode *re) {
...
template <> void Renderable<Par>::Draw(const Par &par, GFXSceneNode *re) {
  ...
  if (fabs(ta) > FPERROR) {
    ...
    // make small spheres so we can see it
    p1 = Round::sphere(p1, .1);
    p2 = Round::sphere(p2, .1);

    auto mat1 = vsr::xf::mat(p1);
    auto mat2 = vsr::xf::mat(p2);

    sphere.mesh.mode(GL::TS);
    Renderable<MBO>::Draw(sphere, mat1, re);
    Renderable<MBO>::Draw(sphere, mat2, re);
    // too small
...
template <>
void Renderable<DualLine>::Draw(const DualLine &dll, GFXSceneNode *re) {
...
template <>
void Renderable<DualPlane>::Draw(const DualPlane &dlp, GFXSceneNode *re) {
...
template <>
void Renderable<Field<Vec>>::Draw(const Field<Vec> &f, GFXSceneNode *re) {
...
template <>
void Renderable<Field<Pnt>>::Draw(const Field<Pnt> &f, GFXSceneNode *re) {
...
// template void Renderable<vector<Cir>>::Draw(const vector<Cir>&, GFXSceneNode
// * s); template void Renderable<Pnt>::Draw(const Pnt&, GFXSceneNode * s);
...
// template void Renderable<Field<Vec>>::Draw(const Field<Vec>&, GFXSceneNode *
// s); template void Renderable<Field<Pnt>>::Draw(const Field<Pnt>&,
// GFXSceneNode * s);
