...
*/
template <class MAT4> gfx::Mat4f mat(const Rot &r) {

  Vec xi = Vec::x.sp(r);
  Vec yi = Vec::y.sp(r);
  Vec zi = Vec::z.sp(r);

  return gfx::Mat4f(xi[0], xi[1], xi[2], 0, yi[0], yi[1], yi[2], 0, zi[0],
                    zi[1], zi[2], 0, 0, 0, 0, 1);
}
...
gfx::Mat4f mat(const Rot &r, const Vec &v, double s) {
  ...
  return gfx::Mat4f(xi[0] * s, xi[1] * s, xi[2] * s, 0, yi[0] * s, yi[1] * s,
                    yi[2] * s, 0, zi[0] * s, zi[1] * s, zi[2] * s, 0, x, y, z,
                    1);
}
...
gfx::Mat4f mat(const Vec &v, VSR_PRECISION s) {
  ...
  return gfx::Mat4f(s, 0, 0, 0, 0, s, 0, 0, 0, 0, s, 0, x, y, z, 1);
}
...
/*
4x4 Transformation matrix from reference and target
*/
gfx::Mat4f mat(const Vec &va, const Vec &vb) {
  Rot r = Gen::ratio(va, vb.unit());
  return mat(r, vb);
}
