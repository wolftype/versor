 ...
 *
 *    Description:  tangent field take 2 
 *                  1 - (dn is projected on edges not faces)
 *                  2 - this projection is the weight of each reciprocal frame
 *
 *        Version:  1.0
...
struct MyApp : App {    
  ...
  void initGui(){
      gui(xamt,"xamt",-100,100)(rotamt,"rotamt",-100,100);
      gui(wt,"wt",-100,100);
      gui(amt,"amt",-100,100);
  ...
  void cotan(){
    ...
    glEnd();
      ...
      if ( interface.keyboard.down ){
            ...
            for (int i=1;i<fc.w()-1;++i){
              for (int j=1;j<fc.h()-1;++j){
               ...
               // biv = (csb + csc + csd + cse) * dcurve;
                ...
                auto descent = bMean ? f2.at(i,j) : f.at(i,j);
                auto mot2 = Gen::mot( (pa ^ biv.duale()^Inf(1)).dual() * descent );
        ...
        if (interface.keyboard.code == 'r'){
          bReset = !bReset;
          interface.keyboard.code = '0';
        }
      }
  }
};
